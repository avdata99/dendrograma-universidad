$(function() {
    ...
    dataTask.then(dendogram).then(stopSpinner).catch(function (error) {
        throw error;
    });
});
...
function dendogram(facultades) {
    ...
    var results = generateTree(facultades.results, null, 0)[0];
    var directors = getSubordinates(facultades.results, results.data.id);
    ...
    var filterSecretaries = getParameterByName("facultades");
    ...
    var link = svg.selectAll("path.link")
        ...
        .attr("stroke", function (d) {
            return d3.scale.category20().range()[secretaries.indexOf(d.target.nombre)];
        })
        .attr("d", diagonal);
    ...
    node.append("a")
        ...
        .append("circle")
        .attr("r", function (d) {
            var weight = 10;  //d.data.cargo.categoria.orden;
            if (weight) {
        ...
        .attr("stroke", function (d) {
            if (d.tipo == 'universidad') {return 'black';}
            else if (d.tipo == 'facultad') {return 'DarkBlue';}
    ...
    s2.change(function () {
        ...
        if (secretariesAux) {
            if(secretariesAux.length === 1) {
                params.facultades = secretariesAux[0];
            } else {
                params.facultades = "";
                secretariesAux.forEach(function (o) {
                    if(o === secretariesAux[0]) {
                        params.facultades = o; //first element
                    } else {
                        params.facultades = params.facultades + secreatariesSeparator + o;
                    }
                });
            }
        }
